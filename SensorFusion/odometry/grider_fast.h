...
public:
     ...
     * @param mask Region of the image we do not want to extract features in
     * (255 = do not detect features)
    ...
    static void perform_griding(const cv::Mat &img, const cv::Mat &mask,
                                ...
                                int num_features, int grid_x, int grid_y,
        ...
        parallel_for_(
            ...
            LambdaBody([&](const cv::Range &range) {
                for (int r = range.start; r < range.end; r++) {
                    ...
                    cv::FAST(img(img_roi), pts_new, threshold,
                             nonmaxSuppression);
                    ...
                    // extracted it in a ROI So we should append the location of
                    // that ROI in the image
                    for (size_t i = 0;
                         i < (size_t)num_features_grid && i < pts_new.size();
                         i++) {
                        // Create keypoint
                        ...
                        if ((int)pt_cor.pt.x < 0 ||
                            (int)pt_cor.pt.x > img.cols ||
                            (int)pt_cor.pt.y < 0 || (int)pt_cor.pt.y > img.rows)
                            continue;

                        // Check if it is in the mask region
                        // NOTE: mask has max value of 255 (white) if it should
                        // be removed
                        if (mask.at<uint8_t>((int)pt_cor.pt.y,
                                             (int)pt_cor.pt.x) < 127)
                            continue;
        ...
        for (size_t r = 0; r < collection.size(); r++) {
            pts.insert(pts.end(), collection.at(r).begin(),
                       collection.at(r).end());
        }
