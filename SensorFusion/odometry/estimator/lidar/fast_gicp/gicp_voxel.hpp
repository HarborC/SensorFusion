...
neighbor_offsets(NeighborSearchMethod search_method) {
...
class Vector3iHash {
public:
    using Ptr = std::shared_ptr<Vector3iHash>;
...
public:
    ...
    virtual void append(const Eigen::Vector4d& mean_,
                        const Eigen::Matrix4d& cov_) = 0;
...
public:
    using Ptr = std::shared_ptr<MultiplicativeGaussianVoxel>;
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    ...
    virtual void append(const Eigen::Vector4d& mean_,
                        const Eigen::Matrix4d& cov_) override {
        num_points++;
...
public:
    using Ptr = std::shared_ptr<AdditiveGaussianVoxel>;
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
...
public:
    ...
    GaussianVoxelMap(double resolution, const VoxelAccumulationMode& mode =
                                            VoxelAccumulationMode::ADDITIVE)
        : voxel_resolution_(resolution), voxel_mode_(mode) {}
    ...
    void create_voxelmap(
        ...
        for (int i = 0; i < cloud.size(); i++) {
            Eigen::Vector3i coord = voxel_coord(
            ...
            if (found == voxels_.end()) {
                ...
                case VoxelAccumulationMode::ADDITIVE_WEIGHTED:
                    voxel = std::shared_ptr<AdditiveGaussianVoxel>(
                        new AdditiveGaussianVoxel);
                    break;
                case VoxelAccumulationMode::MULTIPLICATIVE:
                    voxel = std::shared_ptr<MultiplicativeGaussianVoxel>(
                        new MultiplicativeGaussianVoxel);
                    break;
                }
            ...
            voxel->append(cloud.at(i).getVector4fMap().template cast<double>(),
                          covs[i]);
        }
    ...
    Eigen::Vector4d voxel_origin(const Eigen::Vector3i& coord) const {
        Eigen::Vector3d origin =
            (coord.template cast<double>().array() + 0.5) * voxel_resolution_;
...
private:
    ...
    using VoxelMap =
        std::unordered_map<Eigen::Vector3i, GaussianVoxel::Ptr, Vector3iHash,
                           std::equal_to<Eigen::Vector3i>,
                           Eigen::aligned_allocator<std::pair<
                               const Eigen::Vector3i, GaussianVoxel::Ptr>>>;
    VoxelMap voxels_;
};
