...
class MulanIO {
public:
    using Ptr = std::shared_ptr<MulanIO>;
    using PointType = PointVelodyne::Point;
    MulanIO(const std::string& dataset_base_path_)
    ...
    LidarData<PointType>::Ptr getOusterData(long timestamp) {
        std::string frame_file = dataset_base_path + "/sensor_data/Ouster/" +
