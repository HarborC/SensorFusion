...
bool LadybugCamera::Parameters::readFromYamlFile(const std::string& filename) {
...
void LadybugCamera::Parameters::writeToYamlFile(
    ...
    fs << "{"
       << "frectified" << m_frectified << "x0rectified" << m_x0rectified
       << "y0rectified" << m_y0rectified << "}";

    fs.release();
}

LadybugCamera::Parameters& LadybugCamera::Parameters::operator=(
    const LadybugCamera::Parameters& other) {
    if (this != &other) {
        ...
        memcpy(m_inv_poly, other.m_inv_poly,
               sizeof(double) * LADYBUG_INV_POLY_SIZE);
    }
...
std::ostream& operator<<(std::ostream& out,
                         const LadybugCamera::Parameters& params) {
...
Camera::ModelType LadybugCamera::modelType(void) const {
    return mParameters.modelType();
}

const std::string& LadybugCamera::cameraName(void) const {
    return mParameters.cameraName();
}

int LadybugCamera::imageWidth(void) const { return mParameters.imageWidth(); }

int LadybugCamera::imageHeight(void) const { return mParameters.imageHeight(); }

void LadybugCamera::estimateIntrinsics(
    const std::vector<double> InnInitial,
    const std::vector<std::vector<double>> Points) {
    Parameters params = getParameters();

    std::cout << "Inn ... ..." << std::endl;
    std::vector<double> Inn, InnInv;
    std::vector<std::vector<double>> pointsForInitial = CalculateDistortPara(
        Points, InnInitial, imageWidth(), imageHeight(), Inn);

    std::cout << "InvInn ... ..." << std::endl;
    CalculateDistortParaInv(Points, InnInitial, imageWidth(), imageHeight(),
                            InnInv, pointsForInitial);
 ...
 */
void LadybugCamera::liftSphere(const Eigen::Vector2d& p,
 ...
 */
void LadybugCamera::liftProjective(const Eigen::Vector2d& p,
                                   Eigen::Vector3d& P) const {
    ...
    //赋值得到每个内参参数
    k1 = mParameters.poly(0);
    k2 = mParameters.poly(1);
    k3 = mParameters.poly(2);
    k4 = mParameters.poly(3);
    p1 = mParameters.poly(4);
    p2 = mParameters.poly(5);
    c1 = mParameters.poly(6);
    c2 = mParameters.poly(7);
    lamda = mParameters.poly(8);
    fdistorted = mParameters.poly(9);
    x0distorted = mParameters.poly(10);
    y0distorted = mParameters.poly(11);
    frectified = mParameters.frectified();
    x0rectified = mParameters.x0rectified();
    y0rectified = mParameters.y0rectified();
    ...
    double Rectifiedx =
        ((fdistorted * xt * std::tan(sqrtr / fdistorted)) / sqrtr);
    double Rectifiedy =
        ((fdistorted * yt * std::tan(sqrtr / fdistorted)) / sqrtr);
    ...
    double dx = x_ * (k1 * r2 + k2 * r2 * r2 + k3 * r2 * r2 * r2 +
                      k4 * r2 * r2 * r2 * r2) +
                2 * p1 * x_ * y_ + p2 * (r2 + 2 * x_ * x_) + c1 * x_ + c2 * y_;
    double dy = y_ * (k1 * r2 + k2 * r2 * r2 + k3 * r2 * r2 * r2 +
                      k4 * r2 * r2 * r2 * r2) +
                p1 * (r2 + 2 * y_ * y_) + 2 * p2 * x_ * y_ + c1 * y_ + c2 * x_;
    ...
    // LadybugPixCoor2ImgPixCoor(xRectTemp, yRectTemp, mImgWidth, mImgHeight,
    // RectifiedPixalx, RectifiedPixaly);
 ...
 */
void LadybugCamera::spaceToPlane(const Eigen::Vector3d& P,
    ...
    x0rectified = mParameters.x0rectified();
    y0rectified = mParameters.y0rectified();
    k1 = mParameters.inv_poly(0);
    k2 = mParameters.inv_poly(1);
    k3 = mParameters.inv_poly(2);
    k4 = mParameters.inv_poly(3);
    p1 = mParameters.inv_poly(4);
    p2 = mParameters.inv_poly(5);
    c1 = mParameters.inv_poly(6);
    c2 = mParameters.inv_poly(7);
    lamda = mParameters.inv_poly(8);
    fdistorted = mParameters.inv_poly(9);
    x0distorted = mParameters.inv_poly(10);
    y0distorted = mParameters.inv_poly(11);
    ...
    // ImgPixCoor2LadybugPixCoor(Pixalx, Pixaly, mImgWidth, mImgHeight,
    // LadybugPixalCoor_x, LadybugPixalCoor_y);
    ...
    double sqrtr = std::sqrt(xt * xt + yt * yt);
    double Distortedx =
        ((frectified * xt * std::atan(sqrtr / frectified)) / sqrtr);
    double Distortedy =
        ((frectified * yt * std::atan(sqrtr / frectified)) / sqrtr);
 ...
 */
void LadybugCamera::undistToPlane(const Eigen::Vector2d& p_u,
                                  Eigen::Vector2d& p) const {
...
cv::Mat LadybugCamera::initUndistortRectifyMap(cv::Mat& map1, cv::Mat& map2,
                                               float fx, float fy,
...
int LadybugCamera::parameterCount(void) const {
    return LADYBUG_CAMERA_NUM_PARAMS;
}

const LadybugCamera::Parameters& LadybugCamera::getParameters(void) const {
    return mParameters;
}

void LadybugCamera::setParameters(const LadybugCamera::Parameters& parameters) {
    mParameters = parameters;
}

void LadybugCamera::readParameters(const std::vector<double>& parameterVec) {
...
void LadybugCamera::writeParameters(std::vector<double>& parameterVec) const {
...
void LadybugCamera::writeParametersToYamlFile(
    const std::string& filename) const {
    mParameters.writeToYamlFile(filename);
}

std::string LadybugCamera::parametersToString(void) const {
...
// Calib
std::vector<std::vector<double>> LadybugCamera::CalculateDistortPara(
    ...
    EstimateF(points, InnInitial, nWidth, nHight, pointsForInitial,
              FocalLengthInitial, lamdaInitial);
    ...
    for (size_t i = 0; i < pointsForInitial.size(); i++) {
        ...
        Air1 << xi * ri2, xi * ri4, xi * ri6, xi * ri8, 2 * xi * yi,
            (ri2 + 2 * xi * xi), xi, yi;
        Air2 << yi * ri2, yi * ri4, yi * ri6, yi * ri8, (ri2 + 2 * yi * yi),
            2 * xi * yi, yi, xi;
        Air.row(2 * i) = Air1;
        Air.row(2 * i + 1) = Air2;
        Dir(2 * i, 0) = (xi - xri);
    ...
    Eigen::Matrix<double, 8, 1> DistInitial =
        (Air.transpose() * Air).inverse() * (Air.transpose() * Dir);

    //����ʼֵ
    double lamda = {lamdaInitial};
    double k[4] = {DistInitial(0), DistInitial(1), DistInitial(2),
                   DistInitial(3)};
    ...
    for (size_t i = 0; i < points.size(); ++i) {
        problem.AddResidualBlock(
            new AutoDiffCostFunction<CostFunctor1, 2, 1, 2, 4, 2, 2>(
                new CostFunctor1(points[i][0], points[i][1], points[i][2],
                                 points[i][3], x0rectified, y0rectified,
                                 FocalLengthInitial)),
            NULL, &lamda, Ip0, k, p, c);
    }
    ...
    result << k[0], k[1], k[2], k[3], p[0], p[1], c[0], c[1], lamda,
        FocalLengthInitial, Ip0[0], Ip0[1], frectified, x0rectified,
        y0rectified;
...
void LadybugCamera::CalculateDistortParaInv(
    std::vector<std::vector<double>> points, std::vector<double> InnInitial,
    ...
    EstimateFInv(points, InnInitial, nWidth, nHight, pointsForInitial1,
                 FocalLengthInitial, lamdaInitial);
    ...
    for (size_t i = 0; i < pointsForInitial.size(); i++) {
        ...
        double ri8 = ri2 * ri2 * ri2 * ri2;
        Eigen::Matrix<double, 1, 8> Air1, Air2;
        Air1 << xri * ri2, xri * ri4, xri * ri6, xri * ri8, 2 * xri * yri,
            (ri2 + 2 * xri * xri), xri, yri;
        Air2 << yri * ri2, yri * ri4, yri * ri6, yri * ri8,
            (ri2 + 2 * yri * yri), 2 * xri * yri, yri, xri;
        Air.row(2 * i) = Air1;
        Air.row(2 * i + 1) = Air2;
        Dir(2 * i, 0) = (xi - xri);
        Dir(2 * i + 1, 0) = (yi - yri);
    ...
    for (size_t i = 0; i < points.size(); ++i) {
        problem.AddResidualBlock(
            new AutoDiffCostFunction<CostFunctor2, 2, 1, 2, 4, 2, 2>(
                new CostFunctor2(points[i][0], points[i][1], points[i][2],
                                 points[i][3], x0rectified, y0rectified,
                                 FocalLengthInitial)),
            NULL, &lamda, Ip0, k, p, c);
    }
...
inline Eigen::Matrix<double, 1, 2> LadybugCamera::Rectifiedxy2Distortedxy(
...
inline Eigen::Matrix<double, 1, 2> LadybugCamera::Distortedxy2Rectifiedxy(
...
inline void LadybugCamera::EstimateF(
    std::vector<std::vector<double>> points, std::vector<double> InnInitial,
    int nWidth, int nHight, std::vector<std::vector<double>>& pointsForInitial,
    ...
    for (double ff = 910; ff < 990; ff = ff + 0.1) {
        ...
        DistortedConner << 0, 0, nWidth - 1, 0, 0, nHight - 1, nWidth - 1,
            nHight - 1;

        for (int i = 0; i < 4; i++) {
            DistortedConner.row(i) = DistortedConner.row(i) - xy0rectified;
            RectifiedConnerC.row(i) =
                Distortedxy2Rectifiedxy(DistortedConner.row(i), ff);
        }

        double maxXDist = std::max(
            DistortedConner(0, 0),
            std::max(DistortedConner(1, 0),
                     std::max(DistortedConner(2, 0), DistortedConner(3, 0))));
        double minXDist = std::min(
            DistortedConner(0, 0),
            std::min(DistortedConner(1, 0),
                     std::min(DistortedConner(2, 0), DistortedConner(3, 0))));
        double maxYDist = std::max(
            DistortedConner(0, 1),
            std::max(DistortedConner(1, 1),
                     std::max(DistortedConner(2, 1), DistortedConner(3, 1))));
        double minYDist = std::min(
            DistortedConner(0, 1),
            std::min(DistortedConner(1, 1),
                     std::min(DistortedConner(2, 1), DistortedConner(3, 1))));

        double maxXRect = std::max(
            RectifiedConnerC(0, 0),
            std::max(RectifiedConnerC(1, 0),
                     std::max(RectifiedConnerC(2, 0), RectifiedConnerC(3, 0))));
        double minXRect = std::min(
            RectifiedConnerC(0, 0),
            std::min(RectifiedConnerC(1, 0),
                     std::min(RectifiedConnerC(2, 0), RectifiedConnerC(3, 0))));
        double maxYRect = std::max(
            RectifiedConnerC(0, 1),
            std::max(RectifiedConnerC(1, 1),
                     std::max(RectifiedConnerC(2, 1), RectifiedConnerC(3, 1))));
        double minYRect = std::min(
            RectifiedConnerC(0, 1),
            std::min(RectifiedConnerC(1, 1),
                     std::min(RectifiedConnerC(2, 1), RectifiedConnerC(3, 1))));
        ...
        for (size_t i = 0; i < points.size(); i++) {
            Distortedxy << points[i][0] - x0rectified,
                points[i][1] - y0rectified;
            RectifiedxyCal = Distortedxy2Rectifiedxy(Distortedxy, ff);
            RectifiedxyCal = RectifiedxyCal * 1.0 * nWidth / RectImgWidth;
            Rectifiedxy << points[i][2] - x0rectified,
                points[i][3] - y0rectified;
    ...
    for (size_t i = 0; i < points.size(); i++) {
        ...
        pfi_[0] = RectifiedxyCal(0, 0);
        pfi_[1] = RectifiedxyCal(0, 1);
        pfi_[2] = Rectifiedxy(0, 0);
        pfi_[3] = Rectifiedxy(0, 1);
        pointsForInitial[i] = pfi_;
    }
}

inline void LadybugCamera::EstimateFInv(
    std::vector<std::vector<double>> points, std::vector<double> InnInitial,
    int nWidth, int nHight, std::vector<std::vector<double>>& pointsForInitial,
    double& f, double& lamda) {
    double x0distorted = InnInitial[0];
    ...
    for (double ff = 390; ff < 410; ff = ff + 0.1) {
        ...
        RectifiedConner << 0, 0, nWidth - 1, 0, 0, nHight - 1, nWidth - 1,
            nHight - 1;

        for (int i = 0; i < 4; i++) {
            RectifiedConner.row(i) = RectifiedConner.row(i) - xy0rectified;
            DistortedConnerC.row(i) =
                Rectifiedxy2Distortedxy(RectifiedConner.row(i), ff);
        }

        double maxXDist = std::max(
            DistortedConnerC(0, 0),
            std::max(DistortedConnerC(1, 0),
                     std::max(DistortedConnerC(2, 0), DistortedConnerC(3, 0))));
        double minXDist = std::min(
            DistortedConnerC(0, 0),
            std::min(DistortedConnerC(1, 0),
                     std::min(DistortedConnerC(2, 0), DistortedConnerC(3, 0))));
        double maxYDist = std::max(
            DistortedConnerC(0, 1),
            std::max(DistortedConnerC(1, 1),
                     std::max(DistortedConnerC(2, 1), DistortedConnerC(3, 1))));
        double minYDist = std::min(
            DistortedConnerC(0, 1),
            std::min(DistortedConnerC(1, 1),
                     std::min(DistortedConnerC(2, 1), DistortedConnerC(3, 1))));

        double maxXRect = std::max(
            RectifiedConner(0, 0),
            std::max(RectifiedConner(1, 0),
                     std::max(RectifiedConner(2, 0), RectifiedConner(3, 0))));
        double minXRect = std::min(
            RectifiedConner(0, 0),
            std::min(RectifiedConner(1, 0),
                     std::min(RectifiedConner(2, 0), RectifiedConner(3, 0))));
        double maxYRect = std::max(
            RectifiedConner(0, 1),
            std::max(RectifiedConner(1, 1),
                     std::max(RectifiedConner(2, 1), RectifiedConner(3, 1))));
        double minYRect = std::min(
            RectifiedConner(0, 1),
            std::min(RectifiedConner(1, 1),
                     std::min(RectifiedConner(2, 1), RectifiedConner(3, 1))));
        int DistortedImgWidth = std::ceil(maxXDist - minXDist);
        int DistortedImgHeight = std::ceil(maxYDist - minYDist);
        double dispt = 0;
        for (size_t i = 0; i < points.size(); i++) {
            Eigen::Matrix<double, 1, 2> Rectifiedxy, DistortedxyCal,
                Distortedxy;
            Rectifiedxy << points[i][2] - x0rectified,
                points[i][3] - y0rectified;
