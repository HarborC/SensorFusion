...
void TrackBase::display_active(cv::Mat &img_out, int r1, int g1, int b1, int r2,
    ...
    for (auto const &pair : img_last_cache) {
        ...
        cv::Mat img_temp;
        if (image_new)
            cv::cvtColor(img_last_cache[pair.first], img_temp,
                         cv::COLOR_GRAY2RGB);
        else
            img_temp = img_out(
                cv::Rect(max_width * index_cam, 0, max_width, max_height));
        ...
        for (size_t i = 0; i < pts_last[pair.first].size(); i++) {
            ...
            cv::circle(img_temp, pt_l, (is_small) ? 1 : 2,
                       cv::Scalar(r1, g1, b1), cv::FILLED);
            ...
            cv::rectangle(img_temp, pt_l_top, pt_l_bot, cv::Scalar(r2, g2, b2),
                          1);
        }
    ...
    for (auto const &pair : img_last_cache) {
        ...
        for (size_t i = 0; i < ids_last[pair.first].size(); i++) {
            ...
            if (std::find(highlighted.begin(), highlighted.end(),
                ...
                cv::Point2f pt_l_top =
                    cv::Point2f(pt_c.x - ((is_small) ? 3 : 5),
                                pt_c.y - ((is_small) ? 3 : 5));
                cv::Point2f pt_l_bot =
                    cv::Point2f(pt_c.x + ((is_small) ? 3 : 5),
                                pt_c.y + ((is_small) ? 3 : 5));
                cv::rectangle(img_temp, pt_l_top, pt_l_bot,
                              cv::Scalar(0, 255, 0), 1);
            ...
            for (size_t z = feat->uvs[pair.first].size() - 1; z > 0; z--) {
                ...
                int color_r = (is_stereo ? b2 : r2) -
                              (int)((is_stereo ? b1 : r1) /
                                    feat->uvs[pair.first].size() * z);
                int color_g = (is_stereo ? r2 : g2) -
                              (int)((is_stereo ? r1 : g1) /
                                    feat->uvs[pair.first].size() * z);
                int color_b = (is_stereo ? g2 : b2) -
                              (int)((is_stereo ? g1 : b1) /
                                    feat->uvs[pair.first].size() * z);
                // Draw current point
                ...
                // If there is a next point, then display the line from this
                // point to the next
                if (z + 1 < feat->uvs[pair.first].size()) {
                    cv::Point2f pt_n(feat->uvs[pair.first].at(z + 1)(0),
                                     feat->uvs[pair.first].at(z + 1)(1));
                    cv::line(img_temp, pt_c, pt_n,
                             cv::Scalar(color_r, color_g, color_b));
                }
