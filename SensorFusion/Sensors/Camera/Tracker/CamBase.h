...
public:
    ...
    CamBase(const camodocal::CameraPtr &cam_p) : camera_ptr(cam_p) {
        if (camera_ptr == nullptr) {
            std::cout << "camera_ptr == nullptr" << std::endl;
            std::exit(EXIT_FAILURE);
        }

        int width = camera_ptr->imageWidth();
        int height = camera_ptr->imageHeight();
        double focal = std::max(width, height);

        // Camera matrix
        cv::Matx33d tempK;
        tempK(0, 0) = focal;
        tempK(0, 1) = 0;
        tempK(0, 2) = (double)width / 2.0;
        tempK(1, 0) = 0;
        tempK(1, 1) = focal;
        tempK(1, 2) = (double)height / 2.0;
        tempK(2, 0) = 0;
        tempK(2, 1) = 0;
        tempK(2, 2) = 1;
    ...
    //  * @param calib Camera calibration information (f_x & f_y & c_x & c_y &
    //  k_1
    //  * & k_2 & k_3 & k_4)
    //  */
    ...
    // /**
    //  * @brief Given a raw uv point, this will undistort it based on the
    //  camera
    ...
    //   Eigen::Vector2f udist_pt;
    //   Eigen::Vector3d udist_p_3d;
    //   camera_ptr->liftProjective(uv_dist, udist_p_3d);
    ...
    Eigen::Vector2d undistort_d(const Eigen::Vector2d &uv_dist) {
        ...
        Eigen::Vector3d udist_p_3d;
        camera_ptr->liftProjective(uv_dist, udist_p_3d);
        ept2(0) = udist_p_3d(0) / udist_p_3d(2);
        ept2(1) = udist_p_3d(1) / udist_p_3d(2);
        return ept2;
    }
    ...
    cv::Point2f undistort_cv(const cv::Point2f &uv_dist) {
        ...
        ept2 = (undistort_d(ept1.cast<double>())).cast<float>();
        cv::Point2f pt_out;
        pt_out.x = ept2(0);
        pt_out.y = ept2(1);
        return pt_out;
    ...
    // /**
    //  * @brief Given a normalized uv coordinate this will distort it to the
    //  raw
    //  * image plane
    ...
    // /**
    //  * @brief Computes the derivative of raw distorted to normalized
    //  coordinate.
    //  * @param uv_norm Normalized coordinates we wish to distort
...
protected:
    ...
    // /// Raw set of camera intrinic values (f_x & f_y & c_x & c_y & k_1 & k_2
    // &
    // /// k_3 & k_4)
