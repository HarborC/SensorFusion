...
public:
    ...
    TrackBase(std::unordered_map<size_t, std::shared_ptr<CamBase>> cameras,
        ...
        // Our current feature ID should be larger then the number of aruco tags
        // we have (each has 4 corners)
     ...
     */
    virtual void display_active(cv::Mat &img_out, int r1, int g1, int b1,
     ...
     * @param r1,g1,b1 first color to draw in
     * @param r2,g2,b2 second color to draw in
     * @param highlighted unique ids which we wish to highlight (e.g. slam
     * feats)
     */
    virtual void display_history(cv::Mat &img_out, int r1, int g1, int b1,
                                 int r2, int g2, int b2,
