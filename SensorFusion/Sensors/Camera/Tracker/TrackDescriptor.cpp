...
void TrackDescriptor::feed_monocular(const CameraData &message, size_t msg_id) {
    ...
    if (pts_last.find(cam_id) == pts_last.end() || pts_last[cam_id].empty()) {
        perform_detection_monocular(img, mask, pts_last[cam_id],
                                    desc_last[cam_id], ids_last[cam_id]);
        img_last[cam_id] = img;
    ...
    // If we have a match new->old then we want to use that ID instead of the
    // new one
    ...
    // printf("[TIME-DESC]: %.4f seconds for
    // detection\n",(rT2-rT1).total_microseconds() * 1e-6); printf("[TIME-DESC]:
    // %.4f seconds for matching\n",(rT3-rT2).total_microseconds() * 1e-6);
    // printf("[TIME-DESC]: %.4f seconds for
    // merging\n",(rT4-rT3).total_microseconds() * 1e-6); printf("[TIME-DESC]:
    // %.4f seconds for feature DB update (%d
    // features)\n",(rT5-rT4).total_microseconds() * 1e-6,
    // (int)good_left.size()); printf("[TIME-DESC]: %.4f seconds for
...
void TrackDescriptor::feed_stereo(const CameraData &message, size_t msg_id_left,
    ...
    if (pts_last[cam_id_left].empty() || pts_last[cam_id_right].empty()) {
        perform_detection_stereo(
            img_left, img_right, mask_left, mask_right, pts_last[cam_id_left],
            pts_last[cam_id_right], desc_last[cam_id_left],
            desc_last[cam_id_right], cam_id_left, cam_id_right,
    ...
    parallel_for_(cv::Range(0, 2), LambdaBody([&](const cv::Range &range) {
                      for (int i = range.start; i < range.end; i++) {
                          ...
                          Grider_FAST::perform_griding(
                              ...
                              is_left ? pts0_ext : pts1_ext, num_features,
...
void TrackDescriptor::robust_match(std::vector<cv::KeyPoint> &pts0,
    ...
    // Loop through all good matches, and only append ones that have passed
    // RANSAC
    for (size_t i = 0; i < matches_good.size(); i++) {
    ...
    for (auto &match1 : matches1) {
        ...
        for (auto &match2 : matches2) {
            ...
            if (match1[0].queryIdx == match2[0].trainIdx &&
                ...
                good_matches.emplace_back(cv::DMatch(match1[0].queryIdx,
                                                     match1[0].trainIdx,
                                                     match1[0].distance));
