...
void TrackAruco::perform_tracking(double timestamp, const cv::Mat &imgin,
    ...
    for (size_t i = 0; i < ids_aruco[cam_id].size(); i++) {
        ...
        for (int n = 0; n < 4; n++) {
            ...
            cv::Point2f npt_l = camera_calib.at(cam_id)->undistort_cv(
                corners[cam_id].at(i).at(n));
            // Append to the ids vector and database
            ids_new.push_back((size_t)ids_aruco[cam_id].at(i) + n * max_tag_id);
            database->update_feature(
                (size_t)ids_aruco[cam_id].at(i) + n * max_tag_id, timestamp,
                cam_id, corners[cam_id].at(i).at(n).x,
    ...
    // Timing information
    // printf("[TIME-ARUCO]: %.4f seconds for
    // detection\n",(rT2-rT1).total_microseconds() * 1e-6);
    // printf("[TIME-ARUCO]: %.4f seconds for feature DB update (%d
    // features)\n",(rT3-rT2).total_microseconds() * 1e-6,
    // (int)good_left.size()); printf("[TIME-ARUCO]: %.4f seconds for
