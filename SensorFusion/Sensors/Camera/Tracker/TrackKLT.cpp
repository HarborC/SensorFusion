...
void TrackKLT::feed_monocular(const CameraData &message, size_t msg_id) {
    ...
    // First we should make that the last images have enough features so we can
    // do KLT This will "top-off" our number of tracks so always have a constant
    // number
    ...
    // If any of our mask is empty, that means we didn't have enough to do
    ...
    for (size_t i = 0; i < pts_left_new.size(); i++) {
        ...
        if ((int)message.masks.at(msg_id).at<uint8_t>(
                (int)pts_left_new.at(i).pt.y, (int)pts_left_new.at(i).pt.x) >
            127)
            continue;
    ...
    // Timing information
    // printf("[TIME-KLT]: %.4f seconds for
    // pyramid\n",(rT2-rT1).total_microseconds() * 1e-6); printf("[TIME-KLT]:
    // %.4f seconds for detection\n",(rT3-rT2).total_microseconds() * 1e-6);
    // printf("[TIME-KLT]: %.4f seconds for temporal
    // klt\n",(rT4-rT3).total_microseconds() * 1e-6); printf("[TIME-KLT]: %.4f
    // seconds for feature DB update (%d
    // features)\n",(rT5-rT4).total_microseconds() * 1e-6,
    // (int)good_left.size()); printf("[TIME-KLT]: %.4f seconds for
...
void TrackKLT::feed_stereo(const CameraData &message, size_t msg_id_left,
    ...
    if (pts_last[cam_id_left].empty() && pts_last[cam_id_right].empty()) {
        ...
        perform_detection_stereo(imgpyr_left, imgpyr_right, mask_left,
                                 mask_right, cam_id_left, cam_id_right,
    ...
    for (size_t i = 0; i < pts_left_new.size(); i++) {
        ...
        if (mask_ll[i] && found_right && mask_rr[index_right]) {
            // Ensure we do not have any bad KLT tracks (i.e., points are
            // negative)
            ...
            // std::cout << "adding to stereo - " << ids_last[cam_id_left].at(i)
            // << " , " << ids_last[cam_id_right].at(index_right) << std::endl;
        } else if (mask_ll[i]) {
            good_left.push_back(pts_left_new.at(i));
            good_ids_left.push_back(ids_last[cam_id_left].at(i));
            // std::cout << "adding to left - " << ids_last[cam_id_left].at(i)
            // << std::endl;
        }
    }
    ...
    for (size_t i = 0; i < pts_right_new.size(); i++) {
        ...
        if (mask_rr[i] && !added_already) {
            ...
            // std::cout << "adding to right - " << ids_last[cam_id_right].at(i)
            // << std::endl;
        }
    }
    ...
    // printf("[TIME-KLT]: %.4f seconds for temporal
    // klt\n",(rT4-rT3).total_microseconds() * 1e-6); printf("[TIME-KLT]: %.4f
    // seconds for stereo klt\n",(rT5-rT4).total_microseconds() * 1e-6);
    // printf("[TIME-KLT]: %.4f seconds for feature DB update (%d
...
void TrackKLT::perform_detection_monocular(const std::vector<cv::Mat> &img0pyr,
    ...
    // Note that we scale this down, so that each grid point is equal to a set
    // of pixels This means that we will reject points that less then
    // grid_px_size points away then existing features
    ...
    while (it0 != pts0.end()) {
        ...
        if (x_grid < 0 || x_grid >= size.width || y_grid < 0 ||
            y_grid >= size.height || x < 0 || x >= img0pyr.at(0).cols ||
            y < 0 || y >= img0pyr.at(0).rows) {
            it0 = pts0.erase(it0);
            it1 = ids0.erase(it1);
            continue;
        }
    ...
    Grider_FAST::perform_griding(img0pyr.at(0), mask0, pts0_ext,
                                 num_featsneeded, grid_x, grid_y, threshold,
                                 true);
...
void TrackKLT::perform_detection_stereo(
    ...
    while (it0 != pts0.end()) {
        ...
        if (x_grid < 0 || x_grid >= size0.width || y_grid < 0 ||
            y_grid >= size0.height || x < 0 || x >= img0pyr.at(0).cols ||
            y < 0 || y >= img0pyr.at(0).rows) {
            it0 = pts0.erase(it0);
            it1 = ids0.erase(it1);
            continue;
    ...
    if (num_featsneeded_0 > std::min(75, (int)(0.2 * num_features))) {
        ...
        std::vector<cv::KeyPoint> pts0_ext;
        Grider_FAST::perform_griding(img0pyr.at(0), mask0, pts0_ext,
                                     num_featsneeded_0, grid_x, grid_y,
                                     threshold, true);
        ...
        // If we have points, do KLT tracking to get the valid projections into
        // the right image
        if (!pts0_new.empty()) {
            // Do our KLT tracking from the left to the right frame of reference
            // Note: we have a pretty big window size here since our projection
            // might be bad Note: but this might cause failure in cases of
            // repeated textures (eg. checkerboard)
            std::vector<uchar> mask;
            // perform_matching(img0pyr, img1pyr, kpts0_new, kpts1_new,
            // cam_id_left, cam_id_right, mask);
            ...
            for (size_t i = 0; i < pts0_new.size(); i++) {
                ...
                // Check to see if it there is already a feature in the right
                // image at this location
                //  1) If this is not already in the right image, then we should
                //  treat it as a stereo 2) Otherwise we will treat this as just
                //  a monocular track of the feature
                // TODO: we should check to see if we can combine this new
                // feature and the one in the right
                // TODO: seems if reject features which overlay with right
                // features already we have very poor tracking perf
                if (mask[i] == 1) {
                    // update the uv coordinates
                    kpts0_new.at(i).pt = pts0_new.at(i);
    ...
    while (it0 != pts1.end()) {
        ...
        if (x_grid < 0 || x_grid >= size1.width || y_grid < 0 ||
            y_grid >= size1.height || x < 0 || x >= img1pyr.at(0).cols ||
            y < 0 || y >= img1pyr.at(0).rows) {
            it0 = pts1.erase(it0);
            it1 = ids1.erase(it1);
            continue;
        }
        ...
        bool is_stereo =
            (std::find(ids0.begin(), ids0.end(), *it1) != ids0.end());
        // Check if this keypoint is near another point
        // NOTE: if it is *not* a stereo point, then we will not delete the
        // feature NOTE: this means we might have a mono and stereo feature near
    ...
    if (num_featsneeded_1 > std::min(75, (int)(0.2 * num_features))) {
        ...
        Grider_FAST::perform_griding(img1pyr.at(0), mask1, pts1_ext,
                                     num_featsneeded_1, grid_x, grid_y,
                                     threshold, true);
