...
protected:
     ...
     * but we also enforce stereo contraints. So we detect features in the left
     * image, and then KLT track them onto the right image. If we have valid
     * tracks, then we have both the keypoint on the left and its matching point
     * in the right image. Will try to always have the "max_features" being
     * tracked through KLT at each timestep.
     */
     ...
     * The two point vectors will be of equal size, but the mask_out variable
     * will specify which points are good or bad. If the second vector is
     * non-empty, it will be used as an initial guess of where the keypoints are
     * in the second image.
     */
    void perform_matching(const std::vector<cv::Mat> &img0pyr,
                          const std::vector<cv::Mat> &img1pyr,
                          std::vector<cv::KeyPoint> &pts0,
                          std::vector<cv::KeyPoint> &pts1, size_t id0,
