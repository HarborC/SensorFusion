...
public:
     ...
     * It will create a new feature, if it is an ID that we have not seen
     * before.
     */
     ...
     * time.
     *
     * This function will return all features that do not a measurement at a
     * time greater than the specified time. For example this could be used to
     * get features that have not been successfully tracked into the newest
     * frame. All features returned will not have any measurements occurring at
     * a time greater then the specified.
     */
    std::vector<std::shared_ptr<Feature>> features_not_containing_newer(
        ...
        // Our vector of features that do not have measurements after the
        // specified time
        ...
        // old
        std::unique_lock<std::mutex> lck(mtx);
        for (auto it = features_idlookup.begin();
             it != features_idlookup.end();) {
            ...
            // If we have a measurement greater-than or equal to the specified,
            // this measurement is find
        ...
        // Debugging
        // std::cout << "feature db size = " << features_idlookup.size() <<
        // std::endl;
     ...
     *
     * This will collect all features that have measurements occurring before
     * the specified timestamp. For example, we would want to remove all
     * features older then the last clone/state in our sliding window.
     */
    ...
    std::vector<std::shared_ptr<Feature>> features_containing(
        ...
        for (auto it = features_idlookup.begin();
            ...
            // Break out if we found a single timestamp that is equal to the
            // specified time
            bool has_timestamp = false;
            for (auto const &pair : (*it).second->timestamps) {
                has_timestamp =
                    (std::find(pair.second.begin(), pair.second.end(),
        ...
        // Debugging
        // std::cout << "feature db size = " << features_idlookup.size() <<
        // std::endl; std::cout << "return vector = " <<
        // feats_has_timestamp.size() << std::endl;
     ...
     *
     * If a feature was unable to be used, it will still remain since it will
     * not have a delete flag set
     */
    void cleanup() {
    ...
    /**
     * @brief Will update the passed database with this database's latest
     * feature information.
     */
    void append_new_measurements(
        ...
        for (const auto &feat : database->get_internal_data()) {
            if (features_idlookup.find(feat.first) != features_idlookup.end()) {
                ...
                std::shared_ptr<Feature> temp =
                    features_idlookup.at(feat.first);
                for (const auto &times : feat.second->timestamps) {
                    ...
                    if (temp->timestamps.find(cam_id) ==
                        temp->timestamps.end()) {
                        temp->timestamps[cam_id] =
                            feat.second->timestamps.at(cam_id);
                        temp->uvs[cam_id] = feat.second->uvs.at(cam_id);
                        temp->uvs_norm[cam_id] =
                            feat.second->uvs_norm.at(cam_id);
                    } else {
                        auto temp_times = temp->timestamps.at(cam_id);
                        for (size_t i = 0;
                             i < feat.second->timestamps.at(cam_id).size();
                             i++) {
                            double time_to_find =
                ...
                // Else we have not found the feature, so lets make it be a new
                // one!
