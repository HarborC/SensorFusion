...
public:
     ...
     * @param numaruco the max id of the arucotags, so we ensure that we start
     * our non-auroc features above this value
     ...
     * @param minpxdist features need to be at least this number pixels away
     * from each other
...
protected:
     ...
     * try to add new features. We return all extracted descriptors here since
     * we DO NOT need to do stereo tracking left to right. Our vector of IDs
     * will be later overwritten when we match features temporally to the
     * previous frame's features. See robust_match() for the matching.
     */
     ...
     *
     * This does the same logic as the perform_detection_monocular() function,
     * but we also enforce stereo contraints. We also do STEREO matching from
     * the left to right, and only return good matches that are found in both
     * the left and right. Our vector of IDs will be later overwritten when we
     * match features temporally to the previous frame's features. See
     * robust_match() for the matching.
     */
     ...
     *
     * This will perform a "robust match" between the two sets of points (slow
     * but has great results). First we do a simple KNN match from 1to2 and
     * 2to1, which is followed by a ratio check and symmetry check. Original
     * code is from the "RobustMatcher" in the opencv examples, and seems to
     * give very good results in the matches.
    ...
    // Minimum pixel distance to be "far away enough" to be a different
    // extracted feature
    int min_px_dist;
