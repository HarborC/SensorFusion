...
#include "flexible_initializer.h"
...
bool FlexibleInitializer::tryIncInit(std::vector<ImuData>& imu_msg_buffer,
    ...
    if (staticInitPtr->tryIncInit(imu_msg_buffer, img_msg)) {
        staticInitPtr->assignInitialState(imu_msg_buffer, m_gyro_old, m_acc_old,
                                          imu_state);
        return true;
    } else if (dynamicInitPtr->tryDynInit(imu_msg_buffer, img_msg)) {
        dynamicInitPtr->assignInitialState(imu_msg_buffer, m_gyro_old,
                                           m_acc_old, imu_state);
        return true;
    }
