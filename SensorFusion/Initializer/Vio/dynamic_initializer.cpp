...
void DynamicInitializer::processIMU(const ImuData& imu_msg) {
    ...
    angular_velocity =
        Tg * (imu_msg.angular_velocity - As * linear_acceleration);
                                         ...
                                         // time
    {
        pre_integrations[frame_count].reset(
            new IntegrationBase{acc_0, gyr_0, Bas[frame_count],
                                Bgs[frame_count], acc_n, acc_w, gyr_n, gyr_w});
    }
...
void DynamicInitializer::slideWindow() {
    if (marginalization_flag == MARGIN_OLD) {
        if (frame_count == WINDOW_SIZE) {
            ...
            pre_integrations[WINDOW_SIZE].reset(new IntegrationBase{
                acc_0, gyr_0, Bas[WINDOW_SIZE], Bgs[WINDOW_SIZE], acc_n, acc_w,
                gyr_n, gyr_w});
