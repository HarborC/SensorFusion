...
bool GlobalSFM::solveFrameByPnP(Matrix3d &R_initial, Vector3d &P_initial, int i,
    ...
    if (int(pts_2_vector.size()) < 15) {
        printf(
            "unstable features tracking, please slowly move you device! or "
            "just put in on the table!\n\n");
    ...
    for (int i = 0; i < (int)sfm_f.size(); i++) {
        if (sfm_f[i].state)
            sfm_tracked_points[sfm_f[i].id] =
                Vector3d(sfm_f[i].position[0], sfm_f[i].position[1],
                         sfm_f[i].position[2]);
    }
    return true;
}
