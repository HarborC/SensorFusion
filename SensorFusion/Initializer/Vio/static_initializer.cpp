...
bool StaticInitializer::tryIncInit(const std::vector<ImuData>& imu_msg_buffer,
    ...
    // calculate feature distance of matched features between prev and curr
    // images
    ...
    for (int i = 0; i < 19; i++)
        itr--;
    double maxDis = *itr;
    // classified as static image if maxDis is smaller than threshold, otherwise
    // reset image counter
    if (maxDis < max_feature_dis) {
        staticImgCounter++;
        init_features.swap(curr_features);
        if (staticImgCounter <
            static_Num)  // return false if number of consecitive static images
                         // does not reach @static_Num
            return false;
    } else {
        //    printf("inclinometer-initializer failed at No.%d static
        //    image.",staticImgCounter+1);
        staticImgCounter = 0;
        return false;
    }
    ...
    // initialize rotation and gyro bias by imu data between the 1st and the
    // No.static_Num image set take_off_stamp as time of the No.static_Num image
...
void StaticInitializer::initializeGravityAndBias(
    const double& time_bound, const std::vector<ImuData>& imu_msg_buffer) {
    ...
    for (const auto& imu_msg : imu_msg_buffer) {
        ...
        sum_angular_vel += Tg * (imu_msg.angular_velocity -
                                 As * Ma * imu_msg.linear_acceleration);
    ...
    // Set rotation
    Quaterniond q0_w_i =
        Quaterniond::FromTwoVectors(gravity_imu, -gravity_world);
