...
void FeatureManager::setRic(const Matrix3d &_ric) { ric = _ric; }
...
bool FeatureManager::addFeatureCheckParallax(int frame_count,
                                             MonoCameraMeasurementPtr image,
                                             double td) {
    ...
    if (parallax_num == 0) {
        ...
        // printf("parallax_sum: %lf, parallax_num: %d", parallax_sum,
        // parallax_num); printf("current parallax: %lf", parallax_sum /
...
void FeatureManager::setDepth(const VectorXd &x) {
    ...
    for (auto &it_per_id : feature) {
        ...
        // printf("feature id %d , start_frame %d, depth %f ",
