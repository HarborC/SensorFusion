...
void Visualizer::DrawImuPose(const Pose& pose) {
    ...
    if (imu_traj_.size() > config_.max_traj_length) { imu_traj_.pop_front(); }
}

void Visualizer::DrawGroundTruth(const Pose& pose) {
    std::lock_guard<std::mutex> lg(data_buffer_mutex_);
    gt_traj_.emplace_back(pose);
    if (gt_traj_.size() > config_.max_traj_length) { gt_traj_.pop_front(); }
}

void Visualizer::DrawGPS(const Pose& pose) {
    std::lock_guard<std::mutex> lg(data_buffer_mutex_);
    gps_points_.emplace_back(pose);
    if (gps_points_.size() > config_.max_gps_length) { gps_points_.pop_front(); }
}

void Visualizer::DrawOdom(const Pose& pose) {
    std::lock_guard<std::mutex> lg(data_buffer_mutex_);
    odom_traj_.emplace_back(pose);
    if (odom_traj_.size() > config_.max_traj_length) { odom_traj_.pop_front(); }
}
...
void Visualizer::Run() {
    ...
    pangolin::View& d_image = pangolin::Display("image")
      .SetBounds(0.0f, 0.5f, 2.0/3.0, 1.0f, config_.img_width / config_.img_height)
